// src/pages/faculty/attendance/MarkAttendancePage.jsx
...
export default function MarkAttendancePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // The roster usually embeds the session. If not, fetch it separately.
      ...
      const students = rosterStudents(roster).map((s, index) => ({
        ...
        name: rowName(s),
        rollNo: rowRollNo(s),
        status: rowCurrentStatus(s), // "" means not marked yet
  ...
  async function handleSave() {
    ...
    try {
      const result = await attendanceService.markSessionAttendance(sessionId, {
        ...
        topic,
        fillMissingAsAbsent: fillMissing,
      });
      ...
      await load(); // reload so cumulative percentages refresh
    } catch (err) {
