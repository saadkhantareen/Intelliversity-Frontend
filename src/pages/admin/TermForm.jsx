    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-xl rounded-2xl mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold text-gray-600">Parent Academic Year</label>
            <select 
          ...
          <div>
            ...
            <input 
              type="text" className="w-full border p-2.5 rounded-lg mt-1"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-t pt-4">
          <div>
            ...
            <select 
              ...
              value={formData.term_type} onChange={e => setFormData({...formData, term_type: e.target.value})}
            >
          ...
          <div>
            ...
            <input type="date" className="w-full border p-2.5 rounded-lg mt-1" value={formData.start_date} onChange={e => setFormData({...formData, start_date: e.target.value})} required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-600">End Date</label>
            <input type="date" className="w-full border p-2.5 rounded-lg mt-1" value={formData.end_date} onChange={e => setFormData({...formData, end_date: e.target.value})} required />
          </div>
        </div>

        <div className="bg-indigo-50 p-6 rounded-xl space-y-4">
          <h3 className="font-bold text-indigo-900 uppercase text-xs tracking-widest">Registration & Deadlines</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="text-[10px] font-bold text-gray-500 uppercase">Reg. Start</label>
              <input type="date" className="w-full border p-2 rounded text-sm" value={formData.course_registration_start_date} onChange={e => setFormData({...formData, course_registration_start_date: e.target.value})} required />
            </div>
            <div>
              <label className="text-[10px] font-bold text-gray-500 uppercase">Reg. End</label>
              <input type="date" className="w-full border p-2 rounded text-sm" value={formData.course_registration_end_date} onChange={e => setFormData({...formData, course_registration_end_date: e.target.value})} required />
            </div>
            <div>
              <label className="text-[10px] font-bold text-red-500 uppercase">Drop Deadline</label>
              <input type="date" className="w-full border p-2 rounded text-sm border-red-100" value={formData.course_drop_deadline} onChange={e => setFormData({...formData, course_drop_deadline: e.target.value})} required />
            </div>
            <div>
              <label className="text-[10px] font-bold text-green-600 uppercase">Fee Deadline</label>
              <input type="date" className="w-full border p-2 rounded text-sm border-green-100" value={formData.fee_deadline} onChange={e => setFormData({...formData, fee_deadline: e.target.value})} required />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center">
          <input 
            type="checkbox" className="w-4 h-4 text-indigo-600 border-gray-300 rounded"
        ...
        <div className="flex justify-end gap-3 pt-6 border-t">
          <button type="button" onClick={() => navigate(-1)} className="px-6 py-2 text-gray-600 hover:underline">Cancel</button>
          <button 
