      ...
      try {
        ...
        if (id) {
          ...
          setFormData({
            ...
            program: typeof curriculumData.program === 'object' 
              ? curriculumData.program.code 
              : (progRes.find(p => p.id === curriculumData.program)?.code || curriculumData.program),
            
            // Convert each course inside the curriculum to a code string
            courses: curriculumData.courses.map(item => ({
              ...item,
              course: typeof item.course === 'object'
                ? item.course.code
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input 
              type="text" className="w-full border border-gray-300 p-2.5 rounded-lg mt-1 outline-none focus:ring-2 focus:ring-indigo-500"
              value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} 
              placeholder="e.g. BSCS Fall 2026 Revised" required 
            />
          </div>
          <div>
            ...
            >
              ...
              {programs.map(p => (
                <option key={p.id} value={p.code}> {/* CRITICAL: Using p.code */}
                  {p.name} ({p.code})
                </option>
              ))}
            </select>
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700">Total Credit Hours Required</label>
          <input 
            type="number" className="w-40 border border-gray-300 p-2.5 rounded-lg mt-1 outline-none focus:ring-2 focus:ring-indigo-500"
            value={formData.total_credit_hours} onChange={e => setFormData({...formData, total_credit_hours: e.target.value})} required 
          />
        </div>
        ...
        <div className="border-t border-gray-100 pt-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              type="button" onClick={addCourseRow}
          ...
          <div className="space-y-3">
            {formData.courses.length === 0 && (
              <p className="text-center text-gray-400 py-4 italic border-2 border-dashed rounded-xl">No courses added to this curriculum yet.</p>
            )}
            {formData.courses.map((row, index) => (
              <div key={index} className="grid grid-cols-12 gap-4 items-end bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm">
                <div className="col-span-5">
                  ...
                  <select 
                    ...
                    value={row.course} onChange={e => updateCourseRow(index, 'course', e.target.value)} required
                  >
                    <option value="">Select Course</option>
                    {courses.map(c => (
                      <option key={c.id} value={c.code}> {/* CRITICAL: Using c.code */}
                        {c.code} - {c.name}
                      </option>
                    ))}
                  </select>
                ...
                <div className="col-span-2">
                  ...
                  <input 
                    type="number" className="w-full border border-gray-300 p-2 rounded-md bg-white mt-1"
                    min="1" value={row.recommended_semester} 
                ...
                <div className="col-span-3">
                  ...
                  <select 
                    ...
                    value={row.course_type} onChange={e => updateCourseRow(index, 'course_type', e.target.value)}
                  >
                ...
                <div className="col-span-2 text-right">
                  <button 
                    type="button" onClick={() => removeCourseRow(index)}
