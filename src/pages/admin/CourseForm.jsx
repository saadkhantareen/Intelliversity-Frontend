...
const CourseForm = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-xl rounded-2xl mt-10 border border-gray-100">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-800">{id ? 'Edit Course' : 'Create New Course'}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Credit Hours (1-10)</label>
            <input 
              type="number" 
              className="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" 
        ...
        <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
          <label className="block text-sm font-bold text-blue-800 mb-2">
            Prerequisites 
            <span className="text-xs font-normal text-blue-600 ml-2">(Hold Ctrl or Cmd to select multiple)</span>
          </label>
          <select 
            multiple 
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {formData.prerequisites.length === 0 ? (
              ...
              formData.prerequisites.map(code => (
                <span key={code} className="bg-blue-600 text-white text-xs px-2.5 py-1 rounded-full font-medium shadow-sm">
                  {code}
                </span>
              ))
            )}
        ...
        <div className="flex justify-end gap-4 pt-6 border-t border-gray-100">
          ...
          <button 
            ...
            className={`px-10 py-2.5 bg-blue-600 text-white rounded-lg font-bold shadow-lg transition ${
              loading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700 active:transform active:scale-95'
            }`}
          >
