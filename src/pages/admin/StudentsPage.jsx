...
function DeleteModal({ student, onConfirm, onCancel, loading }) {
  ...
  const name = `${student.base_profile?.user?.first_name ?? ""} ${student.base_profile?.user?.last_name ?? ""}`.trim();
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Delete student?</h3>
        <p className="text-sm text-gray-500 mb-6">
          This will permanently delete <span className="font-medium text-gray-800">{name}</span> ({student.registration_id}). This action cannot be undone.
        </p>
...
function EditModal({ student, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success("Student updated successfully!", { id: toastId, duration: 3000 });
      onSaved(updated);
    } catch (err) {
...
export default function StudentsPage() {
  ...
  const fetchStudents = useCallback(async () => {
    setFetching(true);
    try {
      const res = await ProfileService.getStudents({ page, page_size: PAGE_SIZE, search: debouncedSearch });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {showCreate && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            ...
            <StudentForm
              onSuccess={() => { setShowCreate(false); fetchStudents(); }}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Section</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">CGPA</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {fetching ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-16 text-gray-400 text-sm">No students found.</td></tr>
                ) : students.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-gray-600 font-mono text-xs">{s.registration_id}</td>
