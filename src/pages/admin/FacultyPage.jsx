...
function DeleteModal({ faculty, onConfirm, onCancel, loading }) {
  ...
  const name = `${faculty.base_profile?.user?.first_name ?? ""} ${faculty.base_profile?.user?.last_name ?? ""}`.trim();
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Delete faculty member?</h3>
        <p className="text-sm text-gray-500 mb-6">
          This will permanently delete <span className="font-medium text-gray-800">{name}</span> ({faculty.registration_id}). This action cannot be undone.
        </p>
...
function EditModal({ faculty, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success("Faculty updated successfully!", { id: toastId, duration: 3000 });
      onSaved(updated);
    } catch (err) {
      toast.error(err.response?.data?.detail ?? "Failed to update.", { id: toastId, duration: 4000 });
    } finally {
      setSaving(false);
    }
  }
...
export default function FacultyPage() {
  ...
  const fetchFaculty = useCallback(async () => {
    setFetching(true);
    try {
      const res = await ProfileService.getFaculty({ page, page_size: PAGE_SIZE, search: debouncedSearch });
      if (Array.isArray(res)) { setFaculty(res); setTotal(res.length); }
      else { setFaculty(res.results ?? []); setTotal(res.count ?? 0); }
    } catch {
  ...
  const getName = (f) => `${f.base_profile?.user?.first_name ?? ""} ${f.base_profile?.user?.last_name ?? ""}`.trim() || "—";
        ...
        {showCreate && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowCreate(false)} className="text-gray-400 hover:text-gray-600"><CloseIcon /></button>
            </div>
            <FacultyForm
              onSuccess={() => { setShowCreate(false); fetchFaculty(); }}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Reg. ID</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {fetching ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-16 text-gray-400 text-sm">No faculty members found.</td></tr>
                ) : faculty.map((f) => (
                  <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-gray-600 font-mono text-xs">{f.registration_id}</td>
