...
export const getTermTypeLabel = (termType) => {
  return (
    TERM_TYPE_OPTIONS.find((option) => option.value === termType)?.label ??
    termType
  );
};
...
export const validateTermDates = (values) => {
  const ranges = [
    ...
    [
      "fee_issue_date",
      "fee_deadline",
      "The fee deadline must be on or after the fee issue date.",
    ],
    [
  ...
  const invalidRange = ranges.find(([startKey, endKey]) => {
    return (
      values[startKey] && values[endKey] && values[startKey] > values[endKey]
    );
  });
