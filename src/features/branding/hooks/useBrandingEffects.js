...
export function useBrandingEffects() {
  ...
  useEffect(() => {
    const fontFamily = extractFontName(branding?.theme_config?.typography?.font_family);
    const headingFamily = extractFontName(branding?.theme_config?.typography?.heading_family);
